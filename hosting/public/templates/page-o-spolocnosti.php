<!DOCTYPE html>
<html lang="<?= $LANG ?>">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title><?= t('k009') ?> — TS Palletium</title>
<meta name="description" content="<?= t('k094') ?>">
<link rel="icon" type="image/png" href="/assets/img/favicon.png">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@600;700;800&family=Inter:wght@400;500;600&display=swap" rel="stylesheet">
<link rel="stylesheet" href="/css/style.css">
<?= hreflangs('o-spolocnosti.html') ?>
</head>
<body>

<header class="site-header">
  <div class="container header-inner">
    <a href="index.html" class="brand" aria-label="<?= t('k003') ?>">
      <img src="/assets/img/logo.png" alt="TS Palletium — Built on Trust">
    </a>
    <nav class="main-nav" aria-label="<?= t('k002') ?>">
      <a href="index.html"><?= t('k001') ?></a>
      <a href="o-spolocnosti.html" class="active"><?= t('k009') ?></a>
      <a href="produkty.html"><?= t('k007') ?></a>
      <a href="kontakt.html"><?= t('k178') ?></a>
      <a href="dopyt.html" class="mobile-cta"><?= t('k006') ?></a>
    </nav>
    <?= lang_switcher('o-spolocnosti.html') ?>
    <div class="header-cta">
      <a href="dopyt.html" class="btn btn-gold"><?= t('k006') ?></a>
    </div>
    <button class="nav-toggle" aria-label="<?= t('k004') ?>" aria-expanded="false">
      <span></span><span></span><span></span>
    </button>
  </div>
</header>

<main>

  <section class="page-hero">
    <div class="bg" style="background-image:url('/assets/img/hero-building.jpg');"></div>
    <div class="container">
      <div class="breadcrumb"><a href="index.html"><?= t('k001') ?></a> / <?= t('k009') ?></div>
      <h1><?= t('k008') ?></h1>
      <p><?= t('k097') ?></p>
    </div>
  </section>

  <!-- KTO SME -->
  <section class="section">
    <div class="container">
      <div class="split reveal">
        <div>
          <h2><?= t('k102') ?></h2>
          <div class="title-rule left"></div>
          <p><?= t('k094') ?></p>
          <p><?= t('k095') ?></p>
          <p><strong><?= t('k098') ?></strong></p>
        </div>
        <div class="media">
          <img src="/assets/img/epal-stacks-closeup.jpg" alt="<?= t('k101') ?>" loading="lazy">
        </div>
      </div>
    </div>
  </section>

  <!-- HLAVNÉ ČINNOSTI -->
  <section class="section dark-2">
    <div class="container">
      <h2 class="section-title reveal"><?= t('k070') ?></h2>
      <div class="title-rule"></div>
      <div class="cards-3">
        <article class="svc-card reveal">
          <div class="thumb"><img src="/assets/img/epal-stacks-closeup.jpg" alt="<?= t('k073') ?>" loading="lazy"></div>
          <div class="body">
            <div class="svc-num">01</div>
            <h3><?= t('k073') ?></h3>
            <p><?= t('k074') ?></p>
          </div>
        </article>
        <article class="svc-card reveal">
          <div class="thumb"><img src="/assets/img/epal-warehouse.jpg" alt="<?= t('k076') ?>" loading="lazy"></div>
          <div class="body">
            <div class="svc-num">02</div>
            <h3><?= t('k076') ?></h3>
            <p><?= t('k077') ?></p>
          </div>
        </article>
        <article class="svc-card reveal">
          <div class="thumb"><img src="/assets/img/forklift-warehouse.jpg" alt="<?= t('k079') ?>" loading="lazy"></div>
          <div class="body">
            <div class="svc-num">03</div>
            <h3><?= t('k079') ?></h3>
            <p><?= t('k080') ?></p>
          </div>
        </article>
      </div>
    </div>
  </section>

  <!-- NÁŠ PRÍSTUP -->
  <section class="section">
    <div class="container">
      <div class="split reveal">
        <div class="media">
          <img src="/assets/img/warehouse-aisles.jpg" alt="<?= t('k111') ?>" loading="lazy">
        </div>
        <div>
          <h2><?= t('k103') ?></h2>
          <div class="title-rule left"></div>
          <p><?= t('k104') ?></p>
          <p><?= t('k105') ?></p>
          <p><?= t('k106') ?></p>
          <ul class="checklist">
            <li><?= t('k107') ?></li>
            <li><?= t('k108') ?></li>
            <li><?= t('k109') ?></li>
            <li><?= t('k110') ?></li>
          </ul>
        </div>
      </div>
    </div>
  </section>

  <!-- PREČO TS PALLETIUM -->
  <section class="section dark-2">
    <div class="container">
      <h2 class="section-title reveal"><?= t('k081') ?></h2>
      <div class="title-rule"></div>
      <div class="split reveal">
        <div>
          <ul class="checklist single">
            <li><?= t('k083') ?></li>
            <li><?= t('k085') ?></li>
            <li><?= t('k086') ?></li>
            <li><?= t('k087') ?></li>
          </ul>
        </div>
        <div>
          <ul class="checklist single">
            <li><?= t('k088') ?></li>
            <li><?= t('k089') ?></li>
            <li><?= t('k090') ?></li>
            <li><?= t('k091') ?></li>
          </ul>
        </div>
      </div>
    </div>
  </section>

  <!-- VÍZIA -->
  <section class="band" id="vizia" style="background-image:url('/assets/img/pallets-sky.jpg');">
    <div class="container">
      <h2 class="reveal"><?= t('k113') ?></h2>
      <p class="reveal"><?= t('k114') ?></p>
      <p class="reveal"><?= t('k115') ?></p>
      <p class="reveal"><?= t('k117') ?></p>
    </div>
  </section>

  <!-- SPOLUPRÁCA -->
  <section class="section">
    <div class="container">
      <h2 class="section-title reveal"><?= t('k129') ?></h2>
      <div class="title-rule"></div>
      <p class="section-lead reveal"><?= t('k121') ?></p>
      <p class="section-lead reveal" style="margin-bottom:0;"><?= t('k122') ?></p>
    </div>
  </section>

  <!-- CTA -->
  <section class="cta-band">
    <div class="container">
      <h2><?= t('k132') ?></h2>
      <p><?= t('k133') ?></p>
      <a href="dopyt.html" class="btn"><?= t('k005') ?> <span class="arr">→</span></a>
    </div>
  </section>

</main>

<!-- FOOTER -->
<footer class="site-footer">
  <div class="container">
    <div class="footer-grid">
      <div class="footer-brand">
        <img src="/assets/img/logo-kruh.png" alt="TS Palletium — Built on Trust">
        <p><?= t('k179') ?></p>
      </div>
      <div>
        <h4><?= t('k180') ?></h4>
        <ul class="footer-links">
          <li><a href="index.html"><?= t('k001') ?></a></li>
          <li><a href="o-spolocnosti.html"><?= t('k009') ?></a></li>
          <li><a href="produkty.html"><?= t('k007') ?></a></li>
          <li><a href="dopyt.html"><?= t('k006') ?></a></li>
          <li><a href="kontakt.html"><?= t('k178') ?></a></li>
        </ul>
      </div>
      <div>
        <h4><?= t('k181') ?></h4>
        <ul class="footer-links">
          <li><a href="obchodne-podmienky.html"><?= t('k182') ?></a></li>
          <li><a href="gdpr.html"><?= t('k183') ?></a></li>
          <li><a href="cookies.html"><?= t('k184') ?></a></li>
          <li><a href="pravne-informacie.html"><?= t('k181') ?></a></li>
        </ul>
      </div>
      <div>
        <h4><?= t('k178') ?></h4>
        <ul class="footer-contact">
          <li>
            <svg viewBox="0 0 24 24" fill="none" stroke-width="1.6"><path d="M12 21s7-5.5 7-11a7 7 0 1 0-14 0c0 5.5 7 11 7 11z"/><circle cx="12" cy="10" r="2.5"/></svg>
            <span>Tom Kave Company s. r. o.<br>Južná trieda 2881/4B<br><?= t('k185') ?></span>
          </li>
          <li>
            <svg viewBox="0 0 24 24" fill="none" stroke-width="1.6"><path d="M4 5l4-1 2 5-2 1a12 12 0 0 0 6 6l1-2 5 2-1 4c-9 1-16-6-15-15z"/></svg>
            <a href="tel:+420704222545">+420 704 222 545</a>
          </li>
          <li>
            <svg viewBox="0 0 24 24" fill="none" stroke-width="1.6"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg>
            <a href="mailto:info@tspalletium.com">info@tspalletium.com</a>
          </li>
        </ul>
      </div>
    </div>
    <div class="footer-bottom">
      <span><?= t('k186') ?></span>
      <span><?= t('k187') ?></span>
    </div>
  </div>
</footer>

<!-- COOKIE BAR -->
<div class="cookie-bar" id="cookieBar" role="dialog" aria-label="<?= t('k193') ?>">
  <h4><?= t('k189') ?></h4>
  <p><?= t('k190') ?></p>
  <div class="cookie-actions">
    <button class="btn btn-gold" id="cookieAcceptAll"><?= t('k191') ?></button>
    <button class="btn btn-outline" id="cookieNecessary"><?= t('k192') ?></button>
  </div>
</div>

<script src="/js/main.js"></script>
<?= view_beacon() ?>
</body>
</html>
